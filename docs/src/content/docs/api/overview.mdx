...
### Generic API (Mastodon / ActivityPub)

Multi-provider API for Mastodon and other ActivityPub instances. Paths are prefixed with `/2/mastodon/{instance}/…`.

- **Base URL**: `https://api.fxembed.com`
- **Spec**: [Generic API Reference](/api/generic/operations/)
- Posts, threads, conversations, profiles, search

## Versioning

All current endpoints are under the `/2/` path prefix (API v2). The legacy v1 endpoints (e.g., `/:handle/status/:id`) still work for backward compatibility but return data through the older embed pipeline.
...
There are no strict per-user rate limits, but you may be blocked if you send excessive requests from an unidentifiable user agent. Use a descriptive `User-Agent` header and be respectful of the service.
...
The interactive API reference pages in this documentation are generated from these specifications.
